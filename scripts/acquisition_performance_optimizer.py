...
def main():
    ...
    for sid in sorted(set(raw_by) | set(raw_rank)):
        ...
        # Semantic precision modifies raw source quality without fully suppressing exploration.
    ...
    for i, row in enumerate(source_rows, 1): row["rank"] = i
    ...
    save(CMD, {
        ...
        "capacity": {"exploitation_pct": 70, "exploration_pct": 20, "strategic_reserve_pct": 10},
        "diagnosed_bottleneck": bottleneck,
        "source_priority": {r["source_id"]: r["recommended_multiplier"] for r in source_rows},
        "top_sources": [r["source_id"] for r in source_rows[:5]],
        "harvest_areas": [r["area_key"] for r in harvest],
        "explore_or_revisit_areas": [r["area_key"] for r in explore[:12]],
        "instruction": "Downstream tasks must use these weights for research depth only; never weaken dedup, route, freshness, legality or truthful-fit gates."
    })
    print(json.dumps({"bottleneck": bottleneck, "sources": [r["source_id"] for r in source_rows[:5]], "semantic_pass": semantic_pass, "hot_plus": hot_plus, "ready": ready_count}))
