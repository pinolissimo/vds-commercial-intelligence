...
def main():
    ...
    radar["explore_or_revisit_next"] = [r for r in radar.get("areas", []) if r.get("mode") in {"REVISIT", "EXPLORATION"}][:20]
    ...
    # HOT first, then semantic score.
    tier = {"HOT+": 3, "HOT": 2, "WARM": 1}
    q.sort(key=lambda x: (tier.get(x.get("priority_tier"), 0), float(x.get("semantic_score") or 0)), reverse=True)
    save(QUEUE, {"schema_version": "1.0", "updated_at": radar.get("updated_at"), "count": len(q), "items": q[:500]})
    print(json.dumps({"unresolved_buckets_guarded": changed, "enrichment_queue": len(q)}))
